...
def get_out_rect_approx_n_sorted_cnts(cnts, slicer, conts_more_than_1 = True): 
    '''
    input: 
...
def get_four_calibration_square(pre_image): 
    ...
    edged = cv2.Canny(gray, 50, 200) # 100 255

    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5)) # 5
    edged = cv2.dilate(edged, kernel)

    # find contour
    cnts = cv2.findContours(edged.copy(), cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE) # CHAIN_APPROX_SIMPLE
    cnts = imutils.grab_contours(cnts) # get the cnts list information, the 1st item of the previous line
    ...
    resized_cnts = cnts_2_ratio(cnts_2b_check, ratio, conts_more_than_1= conts_more_than_1) # conts_more_than_1= conts_more_than_1
    
    name_contour(suitable_shown_image, resized_cnts, conts_more_than_1= conts_more_than_1) # , conts_more_than_1 = conts_more_than_1
    cv2.imshow('image.png', suitable_shown_image)
